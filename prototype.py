...
def rag_compass_query(query: RAGQuery):
    ...
    for item, similarity in ranked:
        ...
        document_results[document_id]["matching_chunks"].append(chunk)
    ...
    if not results:
        return {
            ...
            "user_name": query.user_name,
            "user_security_clearance": query.user_security_clearance,
            "answer": "No accessible documents matched the query. The relevant documents may not have been ingested yet, or the user may not have access.",
            "results": [],
            "retrieval_steps": [
                "Received prompt from internal AI assistant",
                "Checked user access level",
